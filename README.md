...
A implementation of a Redux-inspired store to manage the state of a Tally App. The app increments, decrements, and resets a counter while logging the state changes to the console.

## How to Run

1. Open the `store.js` file in your browser's developer console.
2. You will see logs in the console that simulate the user stories as the counter state changes.

## Overview of Approach

- **State Management**: A central store is created using functional programming principles, which holds the state and allows dispatching actions and subscribing to changes.
- **Reducer**: A simple reducer manages the `ADD`, `SUBTRACT`, and `RESET` actions to update the `count` in the state.
- **Subscription**: The `subscribe` method lets you listen for state changes, logging the new state to the console.

## Challenges

The main challenge was simplifying Redux-like functionality
